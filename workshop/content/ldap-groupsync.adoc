...
wget http://idm.openshiftworkshop.com/pub/ca.crt -O {{ HOME_PATH }}/content/support/ca.crt
oc create configmap ca-config-map --from-file={{ HOME_PATH }}/content/support/ca.crt -n openshift-config
oc apply -f {{ HOME_PATH }}/content/support/oauth-cluster.yaml
----
...
[source,bash,role="execute"]
----
cat {{ HOME_PATH }}/content/support/groupsync.yaml
----
...
[source,bash,role="execute"]
----
oc adm groups sync --sync-config={{ HOME_PATH }}/content/support/groupsync.yaml --confirm
----
